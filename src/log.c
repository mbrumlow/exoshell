...
int
log_check_flag (char *flag, char *check, int len)
{
...
int
log_set_flag (char *flag, int len)
{
  ...
  for (i = 0; i < argc; i++)
      ...
      if (strcmp (argv[i], "-debug") == 0)
	{
	  if (i + 1 < argc)
	    ...
	    {
	      fprintf (stderr,
		       "%s: -log must be followed comma separated list of flags\n",
		       argv[0]);
	      return 0;
	    }
	}
...
void
debug (uint64_t flag, const char *fmt, ...)
{
