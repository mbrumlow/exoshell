...
static void
term_debug (int flag, struct term *t, char *fmt, ...)
{
...
static void
term_write_raw (struct term *t, char *buf, int size)
{
...
static void
term_write (struct term *t, char *fmt, ...)
{
...
static struct line *
term_get_line (struct term *t, int n)
{
  ...
  TERM_DEBUG (TERM_LINES, t,
	      ...
	      n, p, t->next_row, t->next_column, t->li.line, t->li.col,
	      t->li.mark, t->li.watermark);
...
char *
term_get_last_line (struct term *t, int n)
{
...
static void
term_line_set (struct term *t, int diff, int col)
{
...
static void
term_line_next (struct term *t)
{

  if (!term_line_alloc (t))
    return;
...
static void
term_line_cr (struct term *t)
{
  t->li.col = 0;
}

static void
term_line_add (struct term *t, char c)
{
  ...
  if (t->li.col >= line->size)
	...
	return;
      ...
      sgr_buf =
	realloc (line->sgr, sizeof (uint64_t) * ((line->size * 2) + 1));
      if (!sgr_buf)
	return;
      line->sgr = sgr_buf;
...
term_save (struct term *t)
{
  ...
  if (t->check_row != t->cursor_row || t->check_column != t->cursor_column)
    {
      TERM_DEBUG (CURSOR | FIXME | INFO, t,
		  "MISMATCH: row_want: %d, row_have: %d, column_want: %d, column_have: %d\n",
		  t->cursor_row, t->check_row, t->cursor_column,
		  t->check_column);
...
void
term_save_cursor (struct term *t)
{
  term_write (t, "\033[6n");
  read_cursor (t->host, &t->check_row, &t->check_column);
  TERM_DEBUG (CURSOR, t,
	      "term_save_cursor: top: %d, bottom: %d, row: %d, col: %d || nrow: %d, ncol: %d\n",
	      t->top, t->bottom, t->cursor_row, t->cursor_column, t->next_row,
	      t->next_column);
...
void
term_set_extra (struct term *t, const char *buf)
{
  term_unset_extra (t);
  t->extra = strdup (buf);
}
...
void
term_unset_extra (struct term *t)
{
  if (t->extra)
    {
      free (t->extra);
...
term_select (struct term *t)
{
      ...
      for (i = 0; i < 34; i++)
	{
	  if (t->host_decset_low[i] != t->decset_low[i])
		...
		case DECSET_DECTCEM:
		  ...
		  else if (t->decset_low[i] == 2
			   || t->host_decset_low[i] == 1)
		    {
  ...
  term_write (t, "\033[%d;%dH\033[%d;%dr\033[?1;9l\033[%d;%dH",
	      t->cursor_row, t->cursor_column, t->top, t->bottom,
	      t->cursor_row, t->cursor_column);
}

// Clears terminal region. 
void
term_clear (struct term *t)
{
  ...
  if (t->top == 1)
    {
      term_write (t, "\033[%d;%dH\033[K\033[1J\033[%d;%dH", t->bottom, 1, row,
		  col);
    }
  else
    {
      term_write (t, "\033[%d;%dH\033[K\033[0J\033[%d;%dH", t->top, 1, row,
		  col);
    }
}

static inline void
debug_esc (struct term *t)
{
...
static void
term_cursor_wrap (struct term *t)
{
...
static void
term_cursor_next (struct term *t)
{
...
static void
update_state_buf (struct term *t, unsigned char c)
{
  t->buf[t->pos++] = c;
  t->buf[t->pos] = '\0';
}
...
static inline int
esc_check (struct term *t, int state)
{
  if (t->state != state)
    return 1;
  return 0;
...
static int
update (struct term *t, unsigned char c)
{
...
static int
parse_CXX_Param (struct term *t, char stop, int *param)
{
...
static void
filter_CUU (struct term *t)
{

  int param = 0;
  int saved_row;

  if (!parse_CXX_Param (t, 'A', &param))
    return;
...
static void
filter_CUD (struct term *t)
{

  int param = 0;
  int saved_row;

  if (!parse_CXX_Param (t, 'B', &param))
    return;
...
static void
term_filter_DECSET (struct term *t)
{
  ...
  if (!reset && (num == 47 || num == 1047 || num == 1049))
    {
      rewind_esc (t);
      return;
    }
...
static uint8_t
term_sgr_read (struct term *t, int *pos)
{
...
static void
term_set_state_color (struct term *t,
		      ...
		      uint8_t ps3, uint8_t ps4, uint8_t ps5)
{
  ...
  TERM_DEBUG (TERM_SGR, t,
	      "term_set_state_color: ps1: %d, ps2: %d, ps3: %d, ps4: %d; ps5: %d\n",
	      ps1, ps2, ps3, ps4, ps5);
  ...
  if (ps1 == 38)
      ...
      else if (ps2 == 2)
	{
	  sgr[1] |= 0x3;
	  TERM_DEBUG (TERM_SGR, t,
		      "term_set_state_color: fg(24bit): %d,%d,%d\n", ps3, ps4,
		      ps5);
	}
  ...
  if (ps1 == 48)
      ...
      else if (ps2 == 2)
	{
	  sgr[1] |= (0x3 << 4);
	  TERM_DEBUG (TERM_SGR, t,
		      "term_set_state_color: bg(24bit): %d,%d,%d\n", ps3, ps4,
		      ps5);
	}
...
term_filter_SGR (struct term *t)
{
  ...
  do
	...
	{
	  TERM_DEBUG (TERM_SGR, t,
		      "term_filter_SGR: (reset) ps1: %d, pos: %d, max: %d\n",
		      ps1, pos, t->pos);
	  t->sgr = 0;
	  continue;
	}
	...
	{
	  TERM_DEBUG (TERM_SGR, t,
		      "term_filter_SGR: (set:%d) ps1: %d, pos: %d, max: %d\n",
		      ps1 - 1, ps1, pos, t->pos);
	  t->sgr |= 1 << (ps1 - 1);
	  continue;
	}
	...
	{
	  TERM_DEBUG (TERM_SGR, t,
		      "term_filter_SGR: (unset:%d) ps1: %d, pos: %d, max: %d\n",
...
void
term_update (struct term *t, unsigned char *buf, int len)
{
  int i;
  for (i = 0; i < len; i++, buf++)
    {
...
static void
term_draw_sgr (struct term *t, uint64_t new, uint64_t old)
{
  ...
  if (!(n[7] == o[7]) ||
      !(n[6] == o[6]) || !(n[5] == o[6]) || !((n[1] & 0x3) == (o[1] & 0x3)))
    {
      // FG color changed.
...
static int
term_draw_line (struct term *t, int n)
{
...
void
term_resize (struct term *t, int top, int bottom, int columns)
{
  ...
  TERM_DEBUG (TERM, t,
	      "term_resize_A: top: %d -> %d, bot: %d -> %d, cr: %d -> %d, cc: %d -> %d\n",
	      otop, t->top, obot, t->bottom, ocr, t->cursor_row, occ,
	      t->cursor_column);

  term_save (t);

  TERM_DEBUG (TERM, t,
	      "term_resize_B: top: %d -> %d, bot: %d -> %d, cr: %d -> %d, cc: %d -> %d\n",
	      otop, t->top, obot, t->bottom, ocr, t->cursor_row, occ,
	      t->cursor_column);
  ...
  if (t->cursor_row > bottom)
    {
      TERM_DEBUG (TERM | CURSOR, t,
		  "term_resize_C: diff: %d, top: %d -> %d, bot: %d -> %d, cr: %d -> %d, cc: %d -> %d\n",
		  diff, otop, t->top, obot, t->bottom, ocr, t->cursor_row,
		  occ, t->cursor_column);
      ...
      term_write (t, "\033[%d;%dr\033[%dS\033[%d;%dH",
		  t->top, t->bottom, diff, t->cursor_row, t->cursor_column);
  ...
  TERM_DEBUG (TERM, t,
	      "term_resize_D: top: %d -> %d, bot: %d -> %d, cr: %d -> %d, cc: %d -> %d\n",
	      otop, t->top, obot, t->bottom, ocr, t->cursor_row, occ,
	      t->cursor_column);
  ...
  if (t->top == 1)
    {
      term_write (t,
		  "\033[%d;%dH\033[K\033[1J\033[%d;%dH\033[%d;%dr\033[?1;9l\033[%d;%dH",
		  t->bottom, 1, t->top, 1, t->top, t->bottom, t->top, 1);
    }
  else
    {
      term_write (t,
		  "\033[%d;%dH\033[K\033[0J\033[%d;%dH\033[%d;%dr\033[?1;9l\033[%d;%dH",
...
void
term_init (struct term *t, int top, int bottom, int columns, int host,
	   int pty)
{
