...
uint32_t proto_read_req(int fd) {
  uint32_t req;
  if(proto_read_uint32(fd, &req) != PROTO_OK) return PROTO_ERROR;
  return req; 
}
...
uint32_t proto_send_string(int fd, char *s, int len) {
  ...
  if(proto_send_req(fd, PROTO_STRING) != PROTO_OK) return PROTO_ERROR;
  if(proto_send_uint32(fd, l) != PROTO_OK) return PROTO_ERROR;
  if(writefd(fd, s, len) != len) return PROTO_ERROR; 
  return PROTO_OK; 
}
