...
test_cursor_wrap_01 ()
{
  ...
  term_init (&target, 1, wbuf.ws_row, wbuf.ws_col, STDIN_FILENO,
	     STDOUT_FILENO);
      ...
      if (target.check_column != col)
	{
	  ...
	  printf
	    ("cursor_column@%d: %d != %d ws_row=%d, ws_col=%d, check_row=%d, check_column=%d state=%d, pos=%d\n",
	     i, target.check_column, col, wbuf.ws_row, wbuf.ws_col,
	     target.check_row, target.check_column, target.state, target.pos);
	  return 0;
	}
...
test_cursor_wrap_04 ()
{
  ...
  int size = 20;
  term_init (&target, wbuf.ws_row - size + 2, wbuf.ws_row, wbuf.ws_col,
...
int
test_cursor_move_01_subtest (int test, struct term *target,
  ...
  if (target->check_column != col)
    {
      printf
	("\ncursor_column@%d: %d != %d ws_row=%d, ws_col=%d, check_row=%d, check_column=%d state=%d, pos=%d\n",
	 test, target->check_column, col, wbuf->ws_row, wbuf->ws_col,
	 target->check_row, target->check_column, target->state, target->pos);
      return 0;
    }
...
test_cursor_move_01 ()
{
...

  {
    ...
    if (!test_cursor_move_01_subtest
	(i, &target, &buf[0], sizeof (buf) - 1, &wbuf))
      return 0;
  }
...
test_lines_01_subtest (int n)
{
  ...
  if (n < max)
    max = n;			// TODO: fix me, max saved lines is 100. 
  for (i = 0; i < max; i++)
    {
      ...
      if (strcmp (s, &line[0]) != 0)
	{
	  printf ("\nn=%d, test: %d: get: %d, expected '%s' but got '%s'\n",
		  n, i, i + 1, &line[0], s);
	  printf
	    ("line: %d, watermark: %d, mark: %d, wrap: %d, next_row: %d\n",
	     target.li.line, target.li.watermark, target.li.mark,
	     target.li.wrap, target.next_row);
	  return 0;
	}
    }
...
test_run ()
{
  ...
  if (pass && !test_cursor_wrap_01 ())
    pass = 0;
  if (pass && !test_cursor_wrap_02 ())
    pass = 0;
  if (pass && !test_cursor_wrap_03 ())
    pass = 0;
  if (pass && !test_cursor_wrap_04 ())
    pass = 0;
  if (pass && !test_cursor_move_01 ())
    pass = 0;
  if (pass && !test_lines_01 ())
    pass = 0;
  if (pass && !test_lines_02 ())
    pass = 0;
